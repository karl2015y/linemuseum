...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <nav class="col-span-2">
        <ol class="list-reset py-4 pl-2 rounded flex bg-grey-light text-grey">
            ...
            <li class="px-2">/</li>
            <li class="text-gray-400">新增知識點活動頁</li>
        </ol>
    </nav>
...
<form action="{{route('CreateKnowledgeActivity',['museum_id'=>$museum->id])}}" method="post">
    @csrf
    {{-- 知識點活動名稱 --}}
    ...
    {{-- 獲得週期(小時) --}}
    <div class="relative h-10 input-component mb-5 formInputGroup">
        <input id="point_cycle_hour" name="point_cycle_hour" value="{{ old('point_cycle_hour') }}" type="number" required
            class="h-full w-full border-gray-300 px-2 transition-all border-blue rounded-sm" />
    ...
    {{-- 獲得週期(分鐘) --}}
    <div class="relative h-10 input-component mb-5 formInputGroup">
        <input id="point_cycle_min" name="point_cycle_min" value="{{ old('point_cycle_min') }}" type="number" required
            class="h-full w-full border-gray-300 px-2 transition-all border-blue rounded-sm" />
