...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
  ...
  <nav class="col-span-2">
    <ol class="list-reset py-4 pl-2 rounded flex bg-grey-light text-grey">
      ...
      <li class="px-2">/</li> --}}
      <li class="text-gray-400">兌換券列表頁</li>
    </ol>
  </nav>

  {{-- 按鍵區 --}}
  <div class="w-100 text-right mb-2 md:mb-0">
    <a href="{{route('CreateVoucherPage')}}">
    ...
    <tr class="bg-white shadow-md">
      ...
      <td class="p-3">
        <div class="flex align-items-center">
          <div class="ml-3">
            <div class="">{{ $voucher->name }}</div>
            <div class="text-gray-500">{{ $voucher->email }}</div>
          </div>
        </div>
      </td>
      {{-- 建立時間 --}}
      ...
      <td class="p-3">
        <div class="flex align-items-center">
          <div class="ml-3">
            @if ($voucher->CreatedStaff == null)
            <div class="">無</div>
            @else
            <div class=""> {{$voucher->CreatedStaff->name}}</div>
            <div class="text-gray-500">{{ $voucher->CreatedStaff->email }}</div>
            @endif
          </div>
        </div>
      </td>
      {{-- 操作 --}}
      <td class="p-3 text-center">
        <a href="{{route('VucherPage', ['voucher_id'=>$voucher->id ])}}" class="text-gray-800 hover:text-gray-400 mr-2">
