...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
  ...
  <nav class="col-span-2">
    <ol class="list-reset py-4 pl-2 rounded flex bg-grey-light text-grey">
      ...
      <li class="px-2">/</li> --}}
      <li class="text-gray-400">總管人員頁</li>
    </ol>
  </nav>

  {{-- 按鍵區 --}}
  <div class="w-100 text-right mb-2 md:mb-0">
    <a href="{{route('CreateStaffPage')}}">
  ...
  <tbody>
    ...
    <tr class="bg-white shadow-md">
      ...
      <td class="p-3">
        <div class="flex align-items-center">
          <div class="ml-3">
            @if ($staff->CreatedStaff == null)
            <div class="">無</div>
            @else
            <div class=""> {{$staff->CreatedStaff->name}}</div>
            <div class="text-gray-500">{{ $staff->CreatedStaff->email }}</div>
            @endif
          </div>
        </div>
      </td>
      {{-- 操作 --}}
      <td class="p-3 text-center">
        <a href="{{route('StaffPage', ['staff_id'=>$staff->id ])}}" class="text-gray-800 hover:text-gray-400 mr-2">
