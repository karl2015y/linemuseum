...
<div class="pb-4 overflow-x-hidden overflow-y-auto px-3">
    ...
    <div class="flex text-sm justify-around py-4 items-center text-gray-400 ">
        <div @click="step=1"
            :class="{'bg-color-main text-white rounded px-2 py-1.5':step==1,'px-2 py-1.5 border-solid border rounded border-gray-500 text-gray-500':step!=1}">
            1.填寫收件人資料</div>
    ...
    {{-- 第一步 --}}
    <div v-if="step==1" class="px-1">
        <div>
            <h1 class="font-bold text-2xl">1.填寫收件人資料</h1>
            {{-- 收件人姓名 --}}
            <div>
            ...
            {{-- 請輸入完整收件地址 --}}
            <div>
                <div class="mt-3 text-gray-500"><span class="text-sm text-red-500">* </span>請輸入完整收件地址</div>
                <div v-if="!address" class="flex gap-3">
                    <select @change="focusCityHandler()" v-model="focusCity"
                        class="mt-2 w-1/2 bg-gray-100 rounded p-1 shadow-inner ">
                        <option value="0" disabled selected>請選擇縣市</option>
                        <option v-for="city in cityAndDis" :value="city">@{{city.name}}
                        </option>
                    </select>
                    <select ref="districtSelect" @change="focusDistrictHandler" v-model="focusDistrict"
                        class="mt-2 w-1/2 bg-gray-100 rounded p-1 shadow-inner ">
                        <option value="0" disabled selected>請選擇鄉鎮市區</option>
                        <template v-if="focusCity">
                            <option v-for="district in focusCity.districts" :value="district">@{{district.zip}}
                                @{{district.name}}
                        </template>
                        </option>
                    </select>
            ...
            {{-- 與上次寄送資料相同 --}}
            <div v-if="lastOrderData.address" class="mt-6">
                <label for="sameDataWithLast">
                    <input @change="sameDataWithLast" id="sameDataWithLast" type="checkbox">
                    <span class="text-color-third">與上次寄送資料相同</span>
                </label>
            </div>
        ...
        <div>
            {{-- 下一步 --}}
            <button @click="step++"
                class="w-10/12 mx-auto mt-16 py-2 px-6 text-white rounded bg-color-main shadow block md:inline-block">下一步</button>
        </div>
    </div>
    {{-- 第二步 --}}
    <div v-if="step==2" class="px-1">
        <div>
            <h1 class="font-bold text-2xl">2.確認資料</h1>
            {{-- 預購商品 --}}
            <div>
        ...
        <div>
            ...
            <button @click="step--"
                class="w-10/12 mx-auto py-2 px-6 rounded mt-3 bg-gray-200 text-gray-400 hover:bg-gray-400 hover:text-white shadow block md:inline-block">返回上一步</button>
        </div>
    </div>
...
<script>
  ...
  data: {
    ...
    lastOrderData:{
        /*@if($last_PVR)*/
        address:"{{$last_PVR->address}}",
        name:"{{$last_PVR->name}}",
        phone:"{{$last_PVR->phone}}",
        email:"{{$last_PVR->email}}",
        /*@else*/
        address:null,
        name:null,
        phone:null,
  ...
  methods: {
    focusCityHandler(){
      this.chooseCity = this.focusCity.name;
      this.focusDistrict = 0;
      this.chooseDistrict = null;
      this.chooseCode = null;
      this.address = "";
    },
    focusDistrictHandler(){
      this.chooseDistrict = this.focusDistrict.name;
      this.chooseCode = this.focusDistrict.zip;
      this.address = this.chooseCode+this.chooseCity+this.chooseDistrict
      this.$refs.addressInputTxt.focus()
    },
    sameDataWithLast(e){
        if(e.target.checked && this.lastOrderData.address){
            this.address = this.lastOrderData.address;
            this.name = this.lastOrderData.name;
            this.phone = this.lastOrderData.phone;
            this.email = this.lastOrderData.email;
        }else{
            this.address = "";
            this.name = "";
            this.phone = "";
  ...
  watch:{
    address: function (val) {
        const vm = this;
        if(val&& vm.chooseCity && vm.chooseDistrict && vm.chooseCode && val.length<(vm.chooseCity.length)+(vm.chooseDistrict.length)+(vm.chooseCode.length)){
            vm.focusDistrict = 0;
            vm.chooseDistrict = null;
            vm.chooseCode = null;
            vm.address = "";
            setTimeout(() => {
                vm.$refs.districtSelect.focus()
            }, 1);    
        }
    },
  },
  created:function(){
     document.querySelector("#scrolldiv").addEventListener('scroll',(e)=>{
      console.log( document.querySelector("#scrolldiv").scrollTop, document.querySelector("#toptitle").clientHeight/2);

        if( document.querySelector("#scrolldiv").scrollTop>=document.querySelector("#toptitle").clientHeight/2){
            document.querySelector("#toptitle").className = "bg-color-sec fixed py-2 shadow text-center top-0 w-full"
             document.querySelector("#toptitleSpace").classList.remove('hidden')
        }else{
            document.querySelector("#toptitle").className = "relative text-center  pb-2 pt-12 bg-color-sec"
             document.querySelector("#toptitleSpace").classList.add('hidden')
        }
    })
  }
})
