...
<div id="toptitle" class="relative text-center  pb-2 pt-12 bg-color-sec">
    ...
    <span class="text-xl font-medium">我的兌換券-預購商品</span>
</div>
...
<div class="px-2 pb-10" style="margin-top:-2px">
    @foreach ($vcrs as $vcr)
        <div class="p-4 border-solid border-t-2 border-gray-300">
            ...
            <div class="mb-1 ml-5 font-light text-sm text-gray-400">購買時間 | {{$vcr->PreVoucherRecord->created_at}}  ({{$vcr->PreVoucherRecord->created_at->diffForHumans()}})</div>
            <div class="my-1 ml-5 font-light text-sm text-gray-400">寄送狀態 | <span class="text-color-third font-bold">{{$vcr->PreVoucherRecord->current_status}}</span></div>
            <div>
